...
class NaiveAgent(game_master.AgentInterface):
    ...
    def take_action(self,history:list[game_master.HistoryStep],available_actions:str, visible_information: str, rules_violation_feedback:Optional[str]=None) -> str:
...
"""}]
        if rules_violation_feedback is not None:
            messages.append({"role":"user", "content":f"Your last action attempt was invalid. Please listen to this feedback and compose a legal action.\n{rules_violation_feedback}"})
